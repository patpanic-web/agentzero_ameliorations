...
| A19 | Enrichissement profils developer/researcher | 🟡 P2 | 🟡 MOYEN | ⏱️ 30min | ✅ TERMINÉ (Sprint 7) | Ajout section Operational Best Practices. ⚠️ Implémentation initiale incorrecte (FR destructive) — corrigée en A25. |
...
| A22 | BMAD Personas Multi-Model : LLM + outils par phase | 🟢 P3 | 🔴 HAUT | ⏱️ ~10h | ✅ TERMINÉ (Sprint 13) | Assigner un LLM optimal + liste d'outils filtrés à chaque persona BMAD. BA/PM/Scrum → LLM léger (~$0.08/1M). Architect/Developer → LLM puissant. Impact estimé : -56% coût/session. Portée universelle via skill project-governance Level 2. |
| A23 | Interface Kanban PO ↔ Agent Zero | 🟢 P3 | 🔴 HAUT | ⏱️ ? | ✅ TERMINÉ (Sprint 13) | Concept : remplacer/compléter l'interface chat par un Kanban board agile. Le PO interagit avec A0 comme avec une équipe (colonnes To Do/In Progress/Done). Sync BACKLOG.md ↔ board visuel. À explorer : intégration outil existant (Trello/GitHub Projects/Notion/Linear) vs interface custom. Phase Analysis requise. |
...
| M1-IMPL | Token Cost Monitor MVP — Collecte métriques de base | 🔴 P1 | 🔴 MAJEUR | ⏱️ 1h | ✅ TERMINÉ (2026-03-08) | Skill créé : SKILL.md + log_usage.py + report.py. Stockage local /a0/usr/data/token-logs/ | Implémentation monitoring minimal: capture usage OpenRouter natif, stockage QNAP (/a0/usr/storage/token-logs/), rapport session simple. **PRÉALABLE** aux optimisations auto G-002→G-006. Approche No-Core-Change: skill JIT + wrapper |
...
- **Sprint 7** (2026-03-06) : "L'Intelligence Adaptive" — A19 (enrichissement profils developer+researcher, ⚠️ corrigé en Sprint 10) + A21 (skill self-audit + scheduled task hebdomadaire)
- **Sprint 10** (2026-03-07) : "La Rigueur" — A25 (correction méthodologie surcharges : original préservé + EN uniquement + ajout non-destructif)
